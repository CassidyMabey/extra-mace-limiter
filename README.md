...
### 🎯 **Extensible Item Blocking**
- **Additional item support** - Configure other items to block like maces
- **Material-based blocking** - Use exact Minecraft item names (DRAGON_EGG, BEACON, etc.)
- **Bundle integration** - Additional blocked items also detected inside bundles
- **Default disabled** - Only maces blocked by default, expand as needed
...
Extend blocking to other items beyond maces:
```yaml
# Block other valuable/special items using Minecraft material names
additional-blocked-items:
  # Examples (disabled by default):
  # - DRAGON_EGG        # Block dragon egg storage
  # - NETHERITE_SWORD   # Block netherite sword storage  
  # - BEACON           # Block beacon storage
  # - NETHER_STAR      # Block nether star storage
  
  # Default: empty list (only maces blocked)
  []
```
**Note:** Items in this list will be blocked in the same way as maces - including inside bundles and all configured storage types.
